...
class GameManager {
  ...
  private willGameEndAfterCurrentTurn(room: Room, gameState: GameState): boolean {
    if (gameState.artistQueue.length === 0) {
      return true;
    }
  ...
  checkGuess(roomId: string, playerId: string, guess: string): boolean {
    ...
    const suppressCorrectGuessSfx =
      everyoneGuessed && this.willGameEndAfterCurrentTurn(room, gameState);
